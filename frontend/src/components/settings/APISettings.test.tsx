...
describe('APISettings', () => {
  ...
  it('shows password input when locked', () => {
    render(<APISettings />)
    expect(screen.getByPlaceholderText('Enter password to unlock')).toBeInTheDocument()
  })

  it('calls unlock with password when unlock button clicked', async () => {
    render(<APISettings />)
    const passwordInput = screen.getByPlaceholderText('Enter password to unlock')
  ...
  it('shows placeholder for API Key when already configured', () => {
    mockLockStatus.has_api_key = true
    render(<APISettings />)
    expect(screen.getByPlaceholderText('Configured (enter a new value to change)')).toBeInTheDocument()
  })
  ...
  it('disables API Key input when unlocked and has_api_key', () => {
    ...
    mockLockStatus.has_api_key = true
    render(<APISettings />)
    const apiKeyInput = screen.getByPlaceholderText('Configured (enter a new value to change)') as HTMLInputElement
  ...
  it('shows password dialog when showPasswordDialog is true', () => {
    ...
    expect(screen.queryByText('Set protection password (min 8 chars, upper + lower + numbers)')).not.toBeInTheDocument()
  })
