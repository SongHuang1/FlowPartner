        ...
        {subagentResults.length > 0 && (
          <div className="mt-3 space-y-1.5">
            {subagentResults.map((result) => {
              const isExpanded = expandedAgent === result.span_id
              return (
                <div key={result.span_id} className="rounded-lg border border-neutral-200 bg-neutral-50 overflow-hidden">
                  <button
                    type="button"
                    onClick={() => setExpandedAgent(isExpanded ? null : result.span_id)}
                  ...
                  >
                    ...
                    {result.status === 'done' && (
                      <span className="text-xs text-green-600 shrink-0">已完成</span>
                    )}
                    {result.status === 'error' && (
                      <span className="text-xs text-red-500 shrink-0">失败</span>
                    )}
                  </button>
